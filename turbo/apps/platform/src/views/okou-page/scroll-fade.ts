...
/**
 * The same fade at the *top* edge, drawn only while the scroller really does
 * have something above.
 *
 ...
 * mask there is a choice, and the Get started connector list makes the other
 * one. The top edge has none: a row sliced flat against nothing is the
 * rendering fault `SCROLL_FADE_X` describes, and it is the only thing between
 * the list and the words that introduce it.
 *
 * Conditional rather than unconditional like `SCROLL_FADE_Y_END`, whose
 * consumers always overflow: at rest the first row starts exactly at the edge,
 * and fading it there would state a hidden row that does not exist. Base UI's
 * `ScrollArea.Root` already publishes the answer as `data-overflow-y-start`, so
 * this reads it off the ancestor instead of measuring the box a second time --
 * the root has to carry `group` for the variant to see it.
 *
 * The 20px matches the end fade, and is spelled literally because Tailwind
 * scans for class names rather than resolving constants. The two are one
