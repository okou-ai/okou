...
import { builtinConnectors$ } from "../../signals/external/connectors.ts";
import { googleDriveCatalogItem$ } from "../../signals/okou-page/connector-catalog-reads.ts";
...
function useGoogleDriveAvailability(
  ...
  const builtinConnectorList =
    connectorListLoadable.state === "hasData"
      ...
      : connectorListLoadable.state === "loading"
        ? lastConnectorList
        : undefined;
  // `null` is a resolved answer (the catalog does not offer Google Drive);
  // `undefined` means it has not resolved or failed.
  const googleDriveCatalogItem =
  ...
  const googleDriveConnector = googleDriveCatalogItem ?? null;
  ...
  return {
    connectorListLoaded:
      ...
      (builtinConnectorList !== undefined &&
        (googleDriveConnected || googleDriveCatalogItem !== undefined)),
