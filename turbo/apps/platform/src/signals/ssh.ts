...
export const acceptSshConflictReview$ = command(
  async ({ get, set }, reviewed: SshDialogState, signal: AbortSignal) => {
    ...
    set(
      reviewedVersion$,
      reviewed.credential?.revision ?? reviewed.connection?.generation ?? null,
    );
    set(conflict$, null);
  },
);
...
const invalidateAfterSshSave$ = command(
  ({ set }, kind: SshDialogState["kind"]) => {
    set(reload$, (value) => {
      return value + 1;
    });
