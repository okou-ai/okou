...
4. On error, classification and recovery selection happen **before** private
   ...
   Reconnect, quota, other non-transient reasons, 401/403 and corrupt
   credentials/history are terminal. Final incomplete output is terminal with
   `output_token_limit`; pending tool continuation still transfers ownership.
