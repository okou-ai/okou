...
export function buildAgentToolsPrompt(args: {
  ...
  return [
    ...
    "- Without an attached generation template, when the user asks to generate anything (supported generation content: built-in image and presentation, plus connector-backed video, talking-avatar video via `avatar-video`, voice/audio, text, code, document, or website), run `okou generate -h`. Run `okou generate <type>` with no generation input to list every provider available for that type. Do not claim support for other generated content.",
    "- Before executing a generation command, run `okou generate <type> -h` and follow its type-specific input flags. Use `--provider built-in` only for image or presentation generation; video, voice/audio, and avatar-video require `--provider <connector>` and the returned connector skill guidance. Okou no longer offers video or avatar templates or built-in video, voice, or avatar generation.",
