  ...
  return [
    ...
    ...(args.cloudBrowserEnabled === true
      ? [
          ...
          "- Okou Browser native input: use `okou browser input-request` only when the form content must be entered by the user personally and you have precisely located supported native controls on the live page (for example, a password or one-time code the agent must not receive). This is a user handoff, not a general form-fill command. Fill ordinary fields yourself through the attached `agent-browser` session when authorized. For unsupported controls or interactive steps such as CAPTCHA or consent, share the exact `okou browser view` URL for direct takeover, explain the step, and ask the user to reply in chat when finished or blocked.",
          "- Browser handoff boundary: after `okou browser input-request` succeeds, return its exact action URL and perform no further Browser operation in this turn. After sharing the Browser viewer for direct takeover, stop using the Browser in this turn.",
        ]
      : []),
    ...
    "- For AWS SigV4 connector failures, run `okou connector check --help` before the URL check and follow its AWS selector guidance. URL and method alone may yield `unknown-endpoint`; use only observed, non-secret request details.",
