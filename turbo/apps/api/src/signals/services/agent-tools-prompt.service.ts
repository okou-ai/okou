  ...
  return [
    ...
    ...(args.cloudBrowserEnabled === true
      ? [
          ...
          "- Browser form input: use `okou browser input-request` only when the user must personally enter values. It supports native form input only, gives the user a dedicated input form, and does not pass their values to the agent in the request or callback. Do not read back their values from the page. Fill ordinary forms with `agent-browser` instead.",
        ]
      : []),
