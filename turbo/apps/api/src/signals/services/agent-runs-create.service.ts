...
function buildAgentToolsPrompt(args: {
  ...
  return [
    ...
    "- Social: use `okou social` for public research, transcripts, summaries, and media downloads; prefer it over the X connector for public X/Twitter research when capabilities are equivalent. Use the X connector for authenticated actions unavailable in `okou social`. Read `okou social --help` and the relevant subcommand's `--help` before use. Use `okou social capabilities [platform] --json` for supported operations and `okou social status [platform] --json` for service health.",
