...
async function executeApiModelTurn(
  ...
  const operation = runObservedApiModelTurn(args, modelSignal);
...
function throwIfAbortedAfterApiTrace(
  signal: AbortSignal,
  ...
  traceContext?.end(signal.reason);
  signal.throwIfAborted();
}
...
const prepareApiFirstTurn$ = command(async function prepareApiFirstTurn(
  ...
  throwIfAbortedAfterApiTrace(signal, langfuseTraceContext);
