...
export const MORNING_BRIEF_CALENDAR_CAPS = Object.freeze({
  calendarListPages: 2,
...
class CollectionState {
  ...
  noteLimit(limit: MorningBriefReaderLimit): void {
 ...
 *
 * Exported so calendar semantics can be exercised against a reader double
 * without an HTTP stack. The authorization decision itself is never faked here;
 * it belongs to the shared reader and is proved through the real preview route.
 */
export async function collectCalendarsWithReader(args: {
  readonly reader: MorningBriefConnectorReader;
  readonly window: MorningBriefCalendarWindow;
 ...
 */
export async function collectMorningBriefCalendar(args: {
  readonly db: Db;
  readonly scope: MorningBriefCollectionScope;
