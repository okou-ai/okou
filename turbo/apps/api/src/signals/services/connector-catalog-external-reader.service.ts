...
export function connectorCatalogBriefsFromSource(
  args: ExternalCatalogSourceArgs & {
    readonly generation: string | undefined;
  },
): readonly PublicConnectorCatalogBrief[] {
  return effectiveConnectors(args).flatMap((entry) => {
    const connector = entry.connector;
    if (
      args.generation !== undefined &&
      !connector.generation.includes(args.generation)
    ) {
      return [];
    }
    return [
