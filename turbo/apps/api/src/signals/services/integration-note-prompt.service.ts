...
function integrationNoteLines(
  ...
  switch (args.triggerSource) {
    ...
    case "agent": {
      return [
        "- Web chat files: use `okou web download-file -h` when a web chat message includes a `[Web file]` block. `okou web upload-file -h` can share a local file back to the web chat user when file delivery is needed. If the user continues a conversation from another integration in Web chat, deliver files here and do not upload them to the earlier integration unless the user explicitly requests it.",
