  ...
  return {
    ...
    mockClerkOrg(actor: ApiTestUser, state: BddOrgState = {}): void {
      ...
      const memberships = members.map((member) => {
        return {
          id: `orgmem_${actor.orgId}_${member.actor.userId}`,
      ...
      context.mocks.clerk.organizations.deleteOrganizationMembership.mockImplementation(
        (input: unknown) => {
          const membership = memberships.find((entry) => {
            return (
              entry.organization.id === recordValue(input, "organizationId") &&
              entry.publicUserData.userId === recordValue(input, "userId")
            );
          });
          if (!membership) {
            throw new Error("Expected a configured Clerk membership to delete");
          }
