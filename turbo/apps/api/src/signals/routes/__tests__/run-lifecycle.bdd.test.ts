...
describe("RUN-01: agent runner context, queue promotion, and skills", () => {
  ...
  it("rechecks the Okou model switch at final run admission", async () => {
    const api = createRunsApi(context);
    const connectors = createConnectorBddApi(context);
    const { actor, agentId } = await entitledRunActor();
    if (!actor.orgId) {
    ...
    const pending = api.requestCreateRun(
      actor,
      {
        agentId,
        prompt: "disable the Okou model before final admission",
        modelProvider: "built-in",
      },
      [400],
    );
    cleanupRequests.push(pending);
    await expect
      .poll(async () => {
        return (await readOrgAdmissionLockState(context)).waiting;
      })
    ...
    expect(rejected.body.error).toStrictEqual({
      message: "This model is not currently available for this workspace.",
      code: "BAD_REQUEST",
    });
  });

  it("fails a queued Okou run when the switch is off at promotion", async () => {
    // Two admitted runs keep the Okou run queued until the switch changes.
    mockEnv("CONCURRENT_RUN_LIMIT_CAP", "2");
    const api = createRunsApi(context);
    const connectors = createConnectorBddApi(context);
    const { actor, agentId } = await entitledRunActor();
    const first = await api.createRun(actor, {
      agentId,
      prompt: "hold first Okou queue slot",
      modelProvider: "anthropic-api-key",
    });
    const second = await api.createRun(actor, {
      agentId,
      prompt: "hold second Okou queue slot",
      modelProvider: "anthropic-api-key",
    });
    ...
    const queued = await api.createRun(actor, {
      agentId,
      prompt: "queued before the Okou model switch closes",
      modelProvider: "built-in",
    });
    expect(queued.status).toBe("queued");
    ...
    await expect(
      readRunFailureReasonFixture(context, queued.runId),
    ).resolves.toBe("unsupported_model");
    expect(
      (await waitForRunQueueLength(api, actor, 0)).body.queue,
    ).toHaveLength(0);
    expect((await api.readRun(actor, second.runId)).status).toBe("pending");
