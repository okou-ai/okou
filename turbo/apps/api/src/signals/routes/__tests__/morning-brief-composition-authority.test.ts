...
describe("Morning Brief exact source selection and retained authority", () => {
  ...
  it("removes material whose grant was withdrawn while a later source was held, and keeps its siblings", async () => {
  ...
  it("proves the exact selected account and refuses to swap it mid-attempt", async () => {
    const fixture = await setupOwner();
    stubProviders({});
  ...
  it("does not adopt an account selected after the attempt was admitted", async () => {
    const fixture = await setupOwner();
