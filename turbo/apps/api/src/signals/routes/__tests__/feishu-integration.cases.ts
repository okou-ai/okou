...
function createFeishuIntegrationFixture(platform: FeishuPlatform) {
  ...
  async function requestFeishuConnectUrl(
    fixture: FeishuInstallationFixture,
    actor = fixture.actor,
    openId = "ou_feishu_user",
  ): Promise<string> {
...
export type FeishuIntegrationTestGroup =
  | "user-deletion"
  | "installation"
  | "oauth-and-events"
...
export function registerFeishuIntegrationTests(
  group: FeishuIntegrationTestGroup,
): void {
  describe.each(["feishu", "lark"] as const)("%s integration", (platform) => {
    ...
    if (group === "user-deletion") {
      it("removes only the deleted user's Feishu connection mapping", async () => {
        ...
        expect(survivorStatus.body).toMatchObject({
          ...
          connectedUserName: "Feishu User",
          installations: [
            expect.objectContaining({ id: fixture.installationId }),
          ],
        });
      });
    }
    ...
    if (group === "installation") {
      ...
      it("converges concurrent managed connector retries after skill publication fails", async () => {
        ...
        const bothSkillUploadsStarted = createDeferredPromise<void>(
          context.signal,
        );
    ...
    if (group === "oauth-and-events") {
      ...
      it("retries a durably admitted Feishu event after dispatch fails", async () => {
        ...
        const event = directMessage(
          appId,
          "retry this Feishu event",
          undefined,
          {
            messageId,
          },
    ...
    // oxlint-disable-next-line vitest/no-conditional-tests -- The entrypoint selects this group before collection.
    if (group === "branding-and-connection") {
      it.each([
        "installation status",
        ...
        async (phase) => {
          ...
          if (phase === "installation status") {
            ...
            expect(unconnectedContent).toContain(
              `To use Okou in ${provider.name}`,
            );
            const connectUrl = requireValue(
      ...
      it("deduplicates unconnected messages and sends one connect link", async () => {
        ...
        expect(
          `${new URL(connectUrl).origin}${new URL(connectUrl).pathname}`,
        ).toBe(`${APP_ORIGIN}${provider.settingsPath}`);
      });
    ...
    if (group === "files-and-history") {
          ...
          expect.objectContaining({
            ...
            userMessage: {
              ...
              parts: [
                ...
                {
                  type: "text",
                  text: expect.stringContaining(feishuFilePrompt),
                },
                {
                  type: "source",
                  kind: platform,
        ...
        async (chatType) => {
          ...
          const thread = requireValue(
            threads.body.events.find((event) => {
              return (
                event.kind === "created" && event.agentId === defaultAgentId
              );
            }),
      ...
      it("preserves agent card replies, artifact links and rich post resources in history", async () => {
        ...
        fixtureState.historyMessages = [
          {
            ...
            mentions: [
              { key: "@_user_1", id: "ou_reviewer", name: "Reviewer" },
            ],
            body: {
              content: JSON.stringify({
                            ...
                            content: JSON.stringify({
                              ...
                              elements: [
                                [
                                  {
                                    tag: "img",
                                    image_key: "img_rendered_preview",
                                  },
                                ],
                              ],
                            }),
        ...
        expect(history).toContain(
          "- SENDER: {id: ou_feishu_user, name: Linghan}",
        );
        expect(history).toContain(
          "@Reviewer (ou_reviewer) check this screenshot",
        );
      ...
      it("renders system message history and retains unreadable message placeholders", async () => {
        ...
        const messages = [
          ...
          },
          {
            content: JSON.stringify({
              template:
                "{from_user} invited {to_chatters}. Thanks, {from_user}.",
    ...
    if (group === "conversation") {
      ...
      it("runs mention-only group requests with thread history and dedupe", async () => {
        ...
        const groupReply = [...fixtureState.outboundMessages]
          ...
          .find((message) => {
            return (
              message.kind === "reply" && message.target === groupMessageId
            );
          });
      ...
      it("attributes mentioned group replies to the triggering user", async () => {
        ...
        expect(
          secondGroupReply ? messageContent(secondGroupReply) : "",
        ).toContain(`Reply to <at id=${secondOpenId}></at>`);
...
export function registerSharedFeishuConversationTests(): void {
  describe("shared Feishu/Lark conversation and queue behavior", () => {
    ...
    it("terminalizes and delivers a queued Feishu admission failure exactly once", async () => {
      ...
      const queuedMessageId = `om_${randomUUID()}`;
      const queuedPayload = directMessage(
        appId,
        queuedPrompt,
        "ou_feishu_user",
        {
      ...
      const deliveredErrors = fixtureState.outboundMessages.filter(
        (message) => {
