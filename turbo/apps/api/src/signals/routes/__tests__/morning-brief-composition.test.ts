...
describe("POST /api/morning-brief/collection-preview/compose", () => {
  ...
  describe("a provider refuses a source read", () => {
    /** Read one source's row from the preview response. */
    function sourceRow(
    ...
    it("never calls a refused GitHub read a quiet morning", async () => {
      ...
      const at = freezeClock();
      // A refused provider has no evidence to keep; its unread gap must not
      // become a quiet morning.
      server.use(
