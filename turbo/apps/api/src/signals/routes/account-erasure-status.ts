...
const createCapability$ = command(async ({ get }, signal: AbortSignal) => {
  const owner = get(authContext$);
  const capability = createAccountErasureStatusCapability(
    owner.userId,
    nowDate(),
  );
...
const readStatus$ = command(async ({ get }, signal: AbortSignal) => {
  ...
  const token = authorization?.startsWith("Bearer ")
    ...
    : "";
  const userId = userIdFromAccountErasureStatusCapability(token, nowDate());
  if (userId === null) {
    return notFound("Account deletion status unavailable");
  }
  const status = await accountErasureStatus(get(db$), userId);
  signal.throwIfAborted();
  return { status: 200 as const, body: { status }, headers: noStore };
});
