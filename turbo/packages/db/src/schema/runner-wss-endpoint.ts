...
export const runnerWssEndpoints = pgTable(
  "runner_wss_endpoints",
  {
  ...
  (table) => {
    return [
      index("runner_wss_endpoints_host_idx").on(table.hostId),
      index("runner_wss_endpoints_lease_idx").on(table.leaseExpiresAt),
    ];
  },
);
