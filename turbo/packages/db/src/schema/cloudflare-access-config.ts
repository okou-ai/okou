...
export const CLOUDFLARE_ACCESS_SCOPES = ["personal", "organization"] as const;
...
export const cloudflareAccessConfigs = pgTable(
  "cloudflare_access_configs",
  {
    ...
    userId: text("user_id"),
    scope: text("scope", { enum: CLOUDFLARE_ACCESS_SCOPES })
      .default("personal")
      .notNull(),
  ...
  (table) => {
    return [
      ...
      unique("uq_cloudflare_access_configs_org_id").on(table.id, table.orgId),
      ...
      check(
        "chk_cloudflare_access_configs_scope_owner",
