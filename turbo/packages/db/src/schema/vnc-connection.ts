  ...
  (table) => {
    return [
      ...
      check(
        ...
        sql`(${table.securityType} IN ('apple_vnc_password', 'apple_dh', 'apple_srp', 'apple_rsa_srp') AND ${table.trustMode} = 'none' AND ${table.caBundle} IS NULL) OR (${table.securityType} NOT IN ('apple_vnc_password', 'apple_dh', 'apple_srp', 'apple_rsa_srp') AND ((${table.trustMode} = 'system' AND ${table.caBundle} IS NULL) OR (${table.trustMode} = 'custom_ca' AND ${table.caBundle} IS NOT NULL AND octet_length(${table.caBundle}) BETWEEN 1 AND 65536)))`,
      ),
    ];
  },
);
