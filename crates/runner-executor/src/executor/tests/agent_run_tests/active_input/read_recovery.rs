...
#[tokio::test]
async fn run_in_sandbox_recovers_tcp_reset_without_a_warning() {
...
#[tokio::test]
async fn run_in_sandbox_warns_on_genuine_read_failures_after_a_transient() {
    let captured = CapturedEvents::default();
    let _subscriber =
        tracing::subscriber::set_default(tracing_subscriber::registry().with(captured.clone()));
    for failure in [
    ...
    for event in failures.iter().filter(|event| event.level == Level::WARN) {
        assert_eq!(event.fields["consecutive_failures"], "2");
    }
}
