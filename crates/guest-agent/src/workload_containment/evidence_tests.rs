...
#[tokio::test]
async fn the_capture_request_carries_only_its_reason() {
    for (reason, expected) in [
        (CaptureReason::Sample, 1),
        (CaptureReason::Cleanup, 1),
        ...
        let evidence = fixture();
        let response = frame(&serde_json::to_vec(&evidence).unwrap());
        let capture = containment.oom_evidence(reason);
        let serve = async {
            assert_eq!(peer.read_u8().await.unwrap(), expected);
            peer.write_all(&response).await.unwrap();
        };
