...
pub(super) async fn remove_non_directory_workspace_cache_entry(
    path: &Path,
...
pub(super) fn secure_workspace_cache_publication_file(path: &Path) -> LifecycleResult<()> {
...
pub(super) async fn sparse_copy(src: &Path, dst: &Path) -> LifecycleResult<()> {
