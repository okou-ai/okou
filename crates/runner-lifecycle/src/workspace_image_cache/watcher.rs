...
#[derive(Debug)]
pub struct WorkspaceCacheChange {
    pub observed_at: tokio::time::Instant,
    pub committed_cache_keys: BTreeSet<String>,
}
...
pub struct WorkspaceCacheWatcher {
...
impl WorkspaceCacheWatcher {
    pub async fn new(cache: WorkspaceImageCache) -> LifecycleResult<Self> {
    ...
    pub async fn reconcile_initial_relevant_entries(
        &mut self,
        cache_keys: &BTreeSet<String>,
    ) -> LifecycleResult<Option<WorkspaceCacheChange>> {
    ...
    pub async fn next_change(&mut self) -> LifecycleResult<WorkspaceCacheChange> {
        loop {
            let mut ready = self
                .inotify
                .readable()
    ...
    fn entry_is_watchable(entry_dir: &Path) -> LifecycleResult<bool> {
...
fn watcher_error(action: &str, error: Errno) -> LifecycleError {
    LifecycleError::Internal(format!("workspace cache watcher {action}: {error}"))
}

fn watcher_io_error(action: &str, kind: std::io::ErrorKind) -> LifecycleError {
    LifecycleError::Internal(format!("workspace cache watcher {action}: {kind:?}"))
}
