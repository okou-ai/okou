...
pub struct WorkspaceImageLease {
...
pub struct WorkspaceImagePromotionContext {
    cache: WorkspaceImageCache,
    cache_key: String,
    entry_lock: Option<WorkspaceEntryLock>,
    run_id: RunId,
...
pub struct WorkspaceSessionHistorySidecarEntryGuard {
    cache: WorkspaceImageCache,
    cache_key: String,
    run_id: RunId,
...
pub struct WorkspaceImagePromotionIdentityFailure {
    pub promotion: WorkspaceImagePromotionContext,
    pub mismatch: WorkspaceImagePromotionIdentityMismatch,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum WorkspaceImagePromotionOutcome {
    Promoted,
    PreservedExisting,
    SkippedUnpublished,
}
...
impl WorkspaceImageCache {
    ...
    pub async fn prepare(&self, request: WorkspaceImagePrepareRequest<'_>) -> WorkspaceImageLease {
    ...
    pub async fn held_workspace_states_for_profiles_after_commits(
        &self,
        profile_image_sizes_bytes: &BTreeMap<&str, u64>,
    ...
    ) -> LifecycleResult<WorkspaceImagePromotionOutcome> {
...
impl WorkspaceImageLease {
    ...
    pub fn result(&self) -> WorkspaceCacheCheckoutResult {
        self.result
    }
    ...
    pub async fn probe_session_history_sidecar(
        &self,
    ...
    pub fn workspace_drive_config(&self) -> Option<sandbox::WorkspaceDriveConfig> {
...
impl WorkspaceImagePromotionContext {
    pub async fn acquire_idle_workspace_reclamation_permit(
        &self,
    ) -> LifecycleResult<tokio::sync::OwnedSemaphorePermit> {
        self.cache.acquire_idle_workspace_reclamation_permit().await
    }

    pub async fn acquire_session_history_sidecar_export_permit(
        &self,
    ) -> LifecycleResult<tokio::sync::OwnedSemaphorePermit> {
        self.cache
    ...
    pub fn terminal_status(&self) -> WorkspaceCacheTerminalStatus {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub async fn promote(&self) -> LifecycleResult<WorkspaceImagePromotionOutcome> {
        self.promote_without_session_history_sidecar().await
    }

    pub async fn try_acquire_session_history_sidecar_entry_guard(
        &self,
    ...
    pub async fn promote_without_session_history_sidecar(
        &self,
    ) -> LifecycleResult<WorkspaceImagePromotionOutcome> {
...
impl WorkspaceSessionHistorySidecarEntryGuard {
    pub fn session_history_sidecar_tmp_path(&self) -> PathBuf {
        self.cache
            .workspace_image_cache_tmp_sidecar(&self.cache_key, self.run_id)
    }

    pub fn session_history_sidecar_source(
        &self,
        tmp_path: PathBuf,
    ...
    pub async fn discard_session_history_sidecar_source(
        &self,
    ...
    pub async fn promote_with_session_history_sidecar(
        &self,
