...
impl WorkspaceImageCache {
    ...
    ) -> LifecycleResult<Option<WorkspaceCacheMetadata>> {
    ...
    pub(super) async fn read_metadata_file(
        &self,
        metadata_path: &Path,
    ) -> LifecycleResult<WorkspaceCacheMetadata> {
        ...
        serde_json::from_slice(&bytes).map_err(|e| {
            LifecycleError::Internal(format!("parse {}: {e}", metadata_path.display()))
        })
    }
    ...
    pub(super) async fn write_metadata(
        ...
        let bytes = serde_json::to_vec_pretty(&metadata)
            .map_err(|e| LifecycleError::Internal(format!("serialize workspace metadata: {e}")))?;
