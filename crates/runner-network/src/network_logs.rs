...
use crate::network_log_transport::{
    NetworkLogHttpClient, NetworkLogRequestContext, NetworkLogTransportFailure,
};
...
enum UploadFailure {
    Other,
    Transport { failure: NetworkLogTransportFailure },
}
...
pub enum NetworkLogUploadSessionOutcome {
    Excluded,
    Success,
    TransportFailure { failure: NetworkLogTransportFailure },
}
...
struct NetworkLogUploadDegradation {
    ...
    latest_failure: NetworkLogTransportFailure,
}
    ...
    fn observe(
        ...
        let latest_failure = self
            ...
            .find_map(|entry| match entry.outcome {
                ...
                EligibleNetworkLogUploadOutcome::TransportFailure { failure } => Some(failure),
            })?;
...
pub struct NetworkLogUploadHealthTracker {
...
impl NetworkLogUploadHealthTracker {
    ...
    pub fn observe(&self, outcome: NetworkLogUploadSessionOutcome) {
        ...
        if let Some(degradation) = degradation {
            error!(
                ...
                failure_kind = degradation.latest_failure.kind,
                failure_cause = degradation.latest_failure.cause,
...
fn eligible_upload_outcome(
    ...
    match outcome {
        ...
        NetworkLogUploadSessionOutcome::TransportFailure { failure } => {
            Some(EligibleNetworkLogUploadOutcome::TransportFailure { failure })
        }
    }
}
...
pub async fn upload_network_logs(
    http: &impl NetworkLogHttpClient,
    run_id: RunId,
    sandbox_token: &str,
    path: &Path,
    ...
    match outcome {
        ...
        UploadOutcome::Failed(UploadFailure::Transport { failure }) => {
            NetworkLogUploadSessionOutcome::TransportFailure { failure }
        }
...
struct NetworkLogBatchUploader<'a> {
    http: &'a dyn NetworkLogHttpClient,
    run_id: RunId,
    sandbox_token: &'a str,
...
impl<'a> NetworkLogBatchUploader<'a> {
    fn new(http: &'a dyn NetworkLogHttpClient, run_id: RunId, sandbox_token: &'a str) -> Self {
        Self {
            http,
            run_id,
            sandbox_token,
    ...
    async fn flush(&mut self) -> BatchUploadOutcome {
        ...
        let request = match self
            .http
            .prepare_network_log_upload(self.sandbox_token, &payload)
        {
            Ok(request) => request,
            Err(e) => {
                warn!(
        ...
        match result {
            ...
            Err(e) => {
                let failure = match &e {
                    crate::network_log_transport::NetworkLogSendError {
                        ...
                        UploadFailure::Transport { failure: *failure }
                    }
                    _ => {
                        warn!(
                            run_id = %self.run_id,
...
mod test_http {
    use super::*;
    use crate::network_log_transport::{NetworkLogSendError, PreparedNetworkLogRequest};
    ...
    impl NetworkLogHttpClient for HttpClient {
        fn prepare_network_log_upload(
            ...
            let request = self
                ...
                .header(
                    api_contracts::generated::constants::client::headers::CLIENT_REQUEST_ID_HEADER,
                    &client_request_id,
                )
                .header(
                    api_contracts::generated::constants::client::headers::CLIENT_SESSION_ID_HEADER,
                    &self.config.client_session_id,
                )
                .header(
                    api_contracts::generated::constants::client::headers::CLIENT_VERSION_HEADER,
                    &client_version,
                )
                .json(payload)
                .build()
            ...
            Ok(Box::new(PreparedTestRequest {
    ...
    impl PreparedNetworkLogRequest for PreparedTestRequest {
        ...
        async fn send(self: Box<Self>) -> Result<reqwest::Response, NetworkLogSendError> {
            self.client
                ...
                .await
                .map_err(|error| NetworkLogSendError {
                    message: format!(
                        "network_logs: send API request failed: {error}; cause={}",
                        if error.is_timeout() {
                            "timeout"
                        } else {
                            "unknown"
                    ...
                    transport: Some(NetworkLogTransportFailure {
...
mod tests {
    ...
    use super::test_http::{HttpClient, HttpClientConfig};
    ...
    fn eligible_transport_failure() -> EligibleNetworkLogUploadOutcome {
        EligibleNetworkLogUploadOutcome::TransportFailure {
            failure: NetworkLogTransportFailure {
                kind: "timeout",
                cause: "timeout",
            },
        }
    ...
    async fn upload_network_logs_returns_on_transport_error() {
        ...
        let failure = match outcome {
            NetworkLogUploadSessionOutcome::TransportFailure { failure } => failure,
            outcome => panic!("expected transport failure, got {outcome:?}"),
        };
        assert_eq!(failed.level, tracing::Level::INFO);
        assert_batch_request_event(failed, 1, "transport_failure");
        assert_event_field(failed, "failure_kind", failure.kind);
        assert_event_field(failed, "failure_cause", failure.cause);
