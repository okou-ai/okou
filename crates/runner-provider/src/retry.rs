...
impl<H> RetryState<H> {
    pub fn new(initial: Duration, max: Duration, max_failures: Option<u32>) -> Self {
        Self {
            handle: None,
            restart_at: None,
            backoff: initial,
...
pub async fn sleep_until_retry(restart_at: &Option<Instant>) {
    match restart_at {
...
pub async fn recv_retry<T, E: std::fmt::Display>(
