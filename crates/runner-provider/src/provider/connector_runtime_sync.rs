...
use crate::connector_registry::{
    ConnectorRuntimeFailCloseOutcome, ConnectorRuntimePublication, ConnectorRuntimeRegistryHandle,
...
#[derive(Clone)]
pub struct ConnectorRuntimeSyncHandle {
...
struct ActiveRunConnectorRuntimeState {
    source_ip: String,
    registry: ConnectorRuntimeRegistryHandle,
...
pub struct ConnectorRuntimeSyncRegistration<'a> {
    pub run_id: RunId,
    pub source_ip: &'a str,
    pub registry: ConnectorRuntimeRegistryHandle,
    pub targets: &'a [ConnectorRuntimeTargetRegistration],
    pub refreshes: Option<&'a HashMap<String, NetworkPolicyRefresh>>,
}
...
impl ConnectorRuntimeSyncHandle {
    ...
    pub async fn register_run(&self, registration: ConnectorRuntimeSyncRegistration<'_>) {
        self.core.register_run(registration).await;
    }

    pub async fn unregister_run(&self, run_id: RunId) {
        self.core.unregister_run(run_id).await;
    }

    pub async fn notify_connector_runtime_sync(
        &self,
        run_id: RunId,
        target: ConnectorRuntimeTarget,
    ) {
    ...
    pub async fn notify_connector_runtime_sync_until_cancelled(
        &self,
        run_id: RunId,
        target: ConnectorRuntimeTarget,
        cancel: &CancellationToken,
    ...
    async fn sync_connector_runtime_batch_for_registration(
        ...
        let response = loop {
            let response = self
                ...
                .await;
            if !transport_retry_attempted && let Err(ProviderError::ApiTransport(error)) = &response
            {
                if !self
    ...
    async fn publish_connector_runtime_response(
        ...
        if !prepared_publications.is_empty() {
            let transaction = tokio::select! {
                ...
                transaction = snapshot.registry.begin_transaction() => transaction,
            };
...
mod tests {
    ...
    use crate::proxy::{ProxyRegistryHandle, SandboxRegistration, TestProxyRegistry};
    ...
    fn assert_connector_runtime_sync_request(request: &str, run_id: &RunId) {
        ...
        let (_, body) = request
            .split_once("\r\n\r\n")
            .unwrap_or_else(|| panic!("request has no header/body boundary: {request}"));
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(body)
                .unwrap_or_else(|error| panic!("invalid request JSON ({error}): {request}")),
            json!({
    ...
    impl ConnectorRuntimeSyncHarness {
        ...
        async fn new_with_api(api: ApiClient, run_id: RunId, connector_slugs: &[&str]) -> Self {
            ...
            let registry =
                TestProxyRegistry::new(registry_path.clone(), dir.path().join("registry.lock"));
    ...
    async fn registered_slack_registry(
        ...
        let registry = TestProxyRegistry::new(registry_path.clone(), lock_path.clone());
    ...
    async fn registered_runtime_registry_with_targets(
        ...
        tokio::fs::write(&registry_path, br#"{"sandboxes":{},"updatedAt":0}"#)
            .await
            .expect("empty registry should be written");
        let registry = TestProxyRegistry::new(
            registry_path.clone(),
    ...
    async fn assert_cancelled_publication_preserves_unregister(
        ...
        drop(lock);
        let registry = TestProxyRegistry::new(harness.registry_path.clone(), lock_path);
    ...
    async fn registry_patch_error_retains_last_known_good_and_retries() {
        ...
        let failing_registry = TestProxyRegistry::new(registry_path.clone(), invalid_lock_path);
        core.inner
            .active_runs
            .lock()
            .await
    ...
    async fn connector_runtime_sync_splits_batches_without_limiting_run_targets() {
        ...
        tokio::fs::write(&registry_path, br#"{"sandboxes":{},"updatedAt":0}"#)
            .await
            .expect("empty registry should be written");
        let registry = TestProxyRegistry::new(registry_path, dir.path().join("registry.lock"));
        let active_run =
