...
pub fn ensure_profile_jobs_dir(group_dir: &Path, profile: &str) -> io::Result<PathBuf> {
...
pub fn ensure_results_dir(group_dir: &Path) -> io::Result<PathBuf> {
    ensure_group_dir(group_dir)?;
...
pub fn validate_results_dir(group_dir: &Path) -> io::Result<PathBuf> {
...
pub fn ensure_claims_dir(group_dir: &Path) -> io::Result<PathBuf> {
    ensure_group_dir(group_dir)?;
...
pub fn validate_claims_dir(group_dir: &Path) -> io::Result<PathBuf> {
    validate_group_dir(group_dir)?;
    let dir = super::claims_dir(group_dir);
...
pub fn ensure_cancels_dir(group_dir: &Path) -> io::Result<PathBuf> {
    ensure_group_dir(group_dir)?;
...
pub fn validate_cancels_dir(group_dir: &Path) -> io::Result<PathBuf> {
    validate_group_dir(group_dir)?;
    let dir = super::cancels_dir(group_dir);
...
pub fn validate_inputs_dir(group_dir: &Path) -> io::Result<PathBuf> {
    validate_group_dir(group_dir)?;
    let inputs_dir = super::inputs_dir(group_dir);
...
pub fn validate_run_inputs_dir(
    group_dir: &Path,
    run_id: runner_types::ids::RunId,
) -> io::Result<PathBuf> {
...
pub fn ensure_group_dir(group_dir: &Path) -> io::Result<()> {
...
pub fn validate_group_dir(group_dir: &Path) -> io::Result<()> {
    host_file::validate_dir(
        group_dir,
        DirMode::SharedTrustedParent,
        "local queue group directory",
...
pub fn write_private_file(path: &Path, bytes: &[u8], context: &str) -> io::Result<()> {
...
pub fn read_private_file(path: &Path, context: &str) -> io::Result<Vec<u8>> {
...
pub fn read_private_file_with_max(
    path: &Path,
    context: &str,
    max_bytes: usize,
) -> io::Result<Vec<u8>> {
...
pub fn private_file_has_content(path: &Path, context: &str) -> io::Result<bool> {
...
pub fn write_private_marker(path: &Path, context: &str) -> io::Result<()> {
    write_private_file(path, b"", context)
}

pub fn create_private_marker(path: &Path, context: &str) -> io::Result<()> {
    open_private_file(path, true, false, context).map(|_| ())
}

pub fn marker_file_exists(path: &Path, context: &str) -> io::Result<bool> {
...
pub fn marker_path_occupied(path: &Path, context: &str) -> io::Result<bool> {
    match std::fs::symlink_metadata(path) {
        Ok(_) => Ok(true),
        Err(e) if e.kind() == io::ErrorKind::NotFound => Ok(false),
        Err(e) => Err(io::Error::new(
...
pub fn open_private_new_file(path: &Path, context: &str) -> io::Result<File> {
