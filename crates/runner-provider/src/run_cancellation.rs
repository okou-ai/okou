...
pub struct DuplicateRunCancellationRegistration;
...
pub struct RunCancellationSignals {
...
impl RunCancellationRegistry {
    ...
    pub async fn handle(&self, run_id: RunId) -> Option<RunCancellationHandle> {
    ...
    pub async fn handles_for(&self, run_ids: &[RunId]) -> HashMap<RunId, RunCancellationHandle> {
    ...
    pub async fn missing_run_ids(&self, run_ids: &[RunId]) -> Vec<RunId> {
        let state = self.inner.lock().await;
        run_ids
            .iter()
            .copied()
    ...
    pub async fn begin_hard_stop(&self) -> Vec<(RunId, RunCancellationHandle)> {
        let mut state = self.inner.lock().await;
...
impl RunCancellationRegistration {
    ...
    pub async fn request_hard_cancellation(&self) -> bool {
...
impl RunCancellationHandle {
    ...
    pub fn signals(&self) -> RunCancellationSignals {
    ...
    pub async fn request_cooperative_user_cancellation(&self) -> bool {
    ...
    pub async fn request_reconciled_cancellation(&self, mode: RunCancellationMode) -> bool {
    ...
    pub async fn transfer_guard(&self) -> OwnedMutexGuard<()> {
    ...
    pub fn try_transfer_guard(&self) -> Option<OwnedMutexGuard<()>> {
...
impl Default for RunCancellationHandle {
    fn default() -> Self {
        Self::new()
    }
}
...
impl RunCancellationSignals {
    /// Clone the aggregate token, cancelled by either cancellation class.
    pub fn any(&self) -> CancellationToken {
        self.any.clone()
    }

    /// Clone the token cancelled by a cooperative-user request.
    pub fn cooperative_user(&self) -> CancellationToken {
    ...
    /// Build signals where a legacy hard-stop token is also the aggregate token.
    pub fn from_hard_token(cancel: CancellationToken) -> Self {
        Self {
            any: cancel.clone(),
