...
| Source-thread reach for `chat_agent_run_context` rows with null `source_user_id`/`source_org_id`                                                                         | Retained for now. On 2026-09-25, 955 rows had a null owner and none of their source threads still exists, so the reach matches nothing. Removing it changes the relational collector contract, which needs a new collector version. That waits until no incomplete job is captured under the current version. On 2026-09-25 one pending user job still is: `catalogue_absent:discord_chat_deliveries` ([#36798](https://github.com/okou-ai/okou/issues/36798)) blocks it. |
