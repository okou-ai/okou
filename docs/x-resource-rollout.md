...
The API tests cover unconditional N + R billing, source replay and shared resource
history. Zero quantities are discarded without source receipts, while persisted
positive sources retain their amount on replay. Resource production, validation,
lifecycle admission and two-date cleanup remain active.
...
contract. This is a deployment compatibility floor, independent of rollout state.
...
The preceding gated API already accepts the unconditional producer's
...
full-count billing for accounts without an enabled override. A Runner rollback may
reduce resource coverage while generic count billing still works; older date-gated
APIs with the setting unset reject v1 uploads.
...
Every accepted resource observation uses N + R billing against the site-wide
daily resource history. There is no user or organization override and no future
UTC activation date to configure. A persisted positive source retains its amount
on replay. A discarded zero observation has no source receipt and is evaluated
again against the retained resource history when retried.
