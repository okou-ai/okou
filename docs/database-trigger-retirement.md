...
release evidence. A–D preparation shipped before migration `1130`; the sections
below retain the writer and repair contracts established by those releases.

## A-D contraction: migration 1130

`1130_retire_prepared_domain_triggers` removes exactly the eight A–D triggers
...
Migration 1130 takes table locks in billing-root → pending-guard → organization
...
are removed; retained private controls stay through the 1130 transition.
The 1130 verification based on main `3bb0520249d198c7ce9c2444ff9e758b4e8ee951`
passed the same 794 cases across 25 current API route files on each schema,
...
completed the migration in 63–86 ms on PostgreSQL 17.10 with the default limits;
...
operation names or mirrors either legacy invitation column. Migration 1130
...
The current-route-only legacy metadata/plan fixtures are retired with 1130.
...
and are removed together by 1130. An existing trigger can create the row
...
The 1130 release boundary above records the prepared artifact and enforced
...
are removed by 1130. They only validated, so they coexisted with the explicit
...
The 1130 contraction uses the recorded prepared serving/background artifact
...
| `signals/routes/test-runtime-state.ts`                           | The two hosted `*-as-previous-api` operations are removed with 1130. Historical writer controls remain in the private compatibility suite.                                                                                                                                                        |
...
preparation releases and are removed by 1130. Historical writer controls stay
...
Migration 1130 removes the shipped trigger/function and retains the guard
