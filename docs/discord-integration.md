...
| Integration prompt guidance  | Slack-triggered runs receive an integration note (`integration-note-prompt.service.ts`): only the final reply reaches the originating thread, Slack commands are for other channels or explicit extra messages, `okou slack download-file`/`upload-file` handle files, and `SLACK_TOKEN` is never used directly. | Discord-triggered runs receive an equivalent note: the canonical callback delivers the final reply, `okou discord message send` is for other channels or extra messages, `okou discord download-file`/`upload-file` handle files, and the bot token is never exposed. The acceptance report found this missing (N4); [#36831](https://github.com/okou-ai/okou/pull/36831) adds it. |
| Welcome guidance             | After an account connects, and the first time a connected user opens the Messages tab, the bot sends one welcome DM (bot identity, workspace agent, how to use). `dmWelcomeSent` deduplicates it.                                                                                                                | OAuth onboarding is deferred, so there is no connect moment and no welcome DM. Until onboarding exists, `/okou help` and the private `connect` response carry the guidance. A once-per-connection welcome is required when onboarding ships; it is a recorded gap, not delivered behavior.                                                                                         |
...
2. Start only the authorized test Gateway. Verify heartbeat/ACK health and
   ...
   discard traffic while the relay still reports `running`. Confirm ordinary guild messages do not start
...
| D: Gateway relay                            | [Merged PR #36660](https://github.com/okou-ai/okou/pull/36660) (`ed449226621bba038c4b92462bbd663293904987`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Authorized test deployment, `deadLettered` alerting, and live relay replay/reconnect acceptance remain required.                          |
...
| Combined-main acceptance fixes              | [Independent report](https://github.com/okou-ai/okou/issues/36819#issuecomment-5826110446) at `a70fca9bebee64e390e86bac66ac221d41401616`. Merged: [#36824](https://github.com/okou-ai/okou/pull/36824) (Known #2, `a70fca9bebee64e390e86bac66ac221d41401616`) and [#36828](https://github.com/okou-ai/okou/pull/36828) (Known #1, `e1786774efd4dadfdf0dcc18e226efb5d5828d64`). Open when this row was written: [#36831](https://github.com/okou-ai/okou/pull/36831) (N4), [#36833](https://github.com/okou-ai/okou/pull/36833) (Known #5), [#36835](https://github.com/okou-ai/okou/pull/36835) (N2), [#36837](https://github.com/okou-ai/okou/pull/36837) (N8), [#36838](https://github.com/okou-ai/okou/pull/36838) (N14), [#36839](https://github.com/okou-ai/okou/pull/36839) (N1), [#36840](https://github.com/okou-ai/okou/pull/36840) (N9–N11), [#36841](https://github.com/okou-ai/okou/pull/36841) (N15). | Track each fix PR to its merge on [#36819](https://github.com/okou-ai/okou/issues/36819); this row is a snapshot, not a live status feed. |
