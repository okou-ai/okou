...
`1248_retire_public_brand` drops `public_brand` from `slack_org_installations`,
...
`link_layout_segment` to lay out records derived from existing content. OAuth and install states are
unaffected; Phase 1 already removed the brand from them.
...
that added `1248_retire_public_brand.sql`. Recovering past that commit requires
