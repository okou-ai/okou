...
The picker's first option, `default`, is the product's own palette rather than a ninth preset. `signals/theme.ts` writes no palette attribute while it is selected, so every token keeps the shared Amber-on-Linen values and none of the `[data-gradient-color-themes]` rules key in — the interface is byte-identical to the one the capability's switch turns off. Its `[data-color-theme="default"]` rule therefore declares only an anchor and a companion, for the one element that does carry the attribute: the picker's own swatch. That pair is the Amber ramp itself — `primary-300` into `primary-500`, crossing `primary-400` at the blend stop — rather than a second hue, because the default palette has no second hue to show. It is also the one swatch whose own card can be the selected one, and the selected card's wash is a pale Amber: a near-white companion such as Linen dissolves the circle's lower edge into that wash, so the companion has to stay darker than the surface it is drawn on. For the same reason `--okou-color-theme-selected`, the wash behind the selected option, is owned at `:root` and refined by the preset rules, rather than existing only under them.
