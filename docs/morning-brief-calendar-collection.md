...
A single calendar's `404`, rate limit, malformed body or oversized body is
local: a valid sibling calendar survives it. Owner, Agent grant, membership,
installation or selected-account invalidation is global — the shared reader
latches it, stops issuing requests and discards the payload instead of
releasing it.

A bounded `Retry-After` is surfaced as metadata. The collector never sleeps on
it and never retries inside a collection.

### Known shared-reader gap

At the shared reader's current head, a provider `403` on a _single calendar_
latches the whole source as `reconnect-required` rather than surfacing the
endpoint-local `denied` outcome. This module already implements and unit-tests
the local-denial path against the documented `denied` outcome; proving it
end-to-end needs the shared fix tracked in
[#34809](https://github.com/vm0-ai/okou/issues/34809). Until then the
route-level suite covers local `404` denial, which is not latched.
...
This module owns Google Calendar semantics only. Every request goes through the
shared Morning Brief connector reader, which resolves the exact selected
account and re-authorizes before credentials, before each next request, and
before the payload is released. Nothing here widens that decision, and the
anchor is the only caller-supplied value on the path — there is no
caller-supplied owner, Agent, account, calendar ID, query or URL.
