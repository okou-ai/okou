...
chain or `activity` deadline profile. The row exists while a runner may still work on the run: launch inserts it,
it is released when a never-started run turns terminal, when the runner reports
completion, or by timeout cleanup, and deleting the run cascades to it.
